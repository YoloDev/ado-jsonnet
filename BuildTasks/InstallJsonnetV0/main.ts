  ...
  new Promise<T>((resolve, reject) => {
    const attempt = (attemptNo: number) => {
      operation().then(resolve, (error) => {
        warning(
          `Operation '${operationName}'#${attemptNo + 1} failed: ${error}`
        );
...
const getLatestRelease = async (github: GitHub): Promise<Release> => {
  ...
  if (result.status !== 200) {
    throw new Error(
      `Operation 'GET /repos/{owner}/{repo}/releases/latest' returned status code ${result.status}`
    );
  }
...
const getTaggedRelease = async (
  ...
  if (result.status !== 200) {
    throw new Error(
      `Operation 'GET /repos/{owner}/{repo}/releases/tags/{tag}' returned status code ${result.status}`
    );
  }
...
const main = async (): Promise<string | false> => {
  ...
  if (!installationPath) {
    installationPath = path.join(
      getVariable("Agent.ToolsDirectory")!,
      "jsonnet"
    );
  }
  ...
  let release: Release;
  if (version) {
    release = await executeWithRetries(
      "getTaggedRelease",
      () => getTaggedRelease(version, github),
      retryLimit
    );
  } else {
    release = await executeWithRetries(
      "getLatestRelease",
      () => getLatestRelease(github),
      retryLimit
    );
  }
  ...
  if (!asset) {
    ...
    error(
      `No valid tarball asset found in release '${release.name}' ('${release.tag_name}')`
    );
    return false;
  }
